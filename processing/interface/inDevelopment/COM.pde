class COM{
   Serial port;
   String ports[]=append(Serial.list(),"select port");
   String portName;
   int indPort=ports.length-1;
   String speeds[]={"115200","250000","2000000","select baud"};
   ...
   //constructor
   COM(Serial port, int xt, int yt, int wt, int ht){
      x=xt; y=yt; w=wt; h=ht;
      u=w/11;
   } 
   void display(){
     ...
     rect(x+5*u,y,4*u,h); 
     rect(x+9*u,y,2*u,h);  
     fill(0);textAlign(CENTER,CENTER); text("configure connection to microcontroller",x+w/2,y-12);
     text(ports[indPort],x+2.5*u,y+h/2); 
     text(speeds[indSpeed],x+7*u,y+h/2); 
     if (connected) tex="on"; else tex="off";
     text(tex,x+10*u,y+h/2);
   }
   ...
   int mouseClicado(){
     ...
     if (mouseY>y && mouseY<y+h){
       ...
       } else if (mouseX>x+9*u && mouseX<x+w){ // mudar X (desconnected) para ok (connected)
